...
| `@deepseek-ai/dsh-experimental-tool-env-inspect` | `apps_diff`、`apps_inspect`、`apps_snapshot`、`env_inspect`、`env_version`、`pkg_inspect`、`pkg_propose` | `ctx.tools`、`ctx.approval`、`ctx.subprocess`、`可选 ctx.shell（apps 工具）` | `tool/call`、`tool/result` | - | env_inspect 与 env_version 是安装器规划的环境探测：输入命令名，输出 PATH 上的可执行路径，并可对每个获批名字运行一次受审批门禁、受死线限制的 --version。apps_inspect 通过可选的 shell 通道从只读的 Windows 注册表、App Paths 与 AppX 来源枚举已安装应用程序；apps_snapshot 捕获一份受界的具名观测，apps_diff 按稳定条目 id 比较两份观测；pkg_inspect 在每个被选中的包管理器各自的审批决定之后运行其固定只读命令，pkg_propose 在安装之前解析一个候选包。这些工具都不运行发现到的程序，也从不返回卸载命令。该包为实验性、被排除在正式发布之外，且没有任何 shipped profile 挂载它；目录记录的上限是目录对必填 config 自行做出的选择。 |
