...
- **Snapshots are bounded and non-durable** — `apps_snapshot` keeps at most `appsMaxSnapshots` named captures in the composition's memory; a host restart loses them, and `apps_diff` fails loud for an unknown name rather than comparing against nothing.
...
- **Nothing here installs anything** — `pkg_propose` reports what a manager says about a candidate and names the hash the source publishes; it never downloads, installs, upgrades, or uninstalls, and DSH does not verify the hash itself. The install/verify/rollback flow is unimplemented by design.
