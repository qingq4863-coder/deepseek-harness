...
A preset may set `capabilityRisk` to cap the tool `risk` it executes. While a ceilinged preset is effective, a monotonic guard denies declared tools whose `capability.risk` ranks above the ceiling and tools without capability metadata; agent-less executions carry no preset selection and no ceiling. Default presets set no ceiling.
