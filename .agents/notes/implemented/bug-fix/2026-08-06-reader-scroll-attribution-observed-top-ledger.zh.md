...
一条可信手势时间戳只回答 ledger 无法回答的一个问题：待处理的滚动采样是否来自读者自身的移动。`readerGestureAtRef` 在滚动容器上记录 `wheel`、`touchstart`、`touchmove`、`pointerdown` 与 `keydown`，而 `followRef` 在采样待处理期间仍跟随内容增长，除非 `READER_GESTURE_WINDOW_MS`（1 秒）内出现过手势——即 [Chat 折叠与重开笔记](2026-09-09-chat-fold-and-reopen-tail.zh.md)修复的那种情形。该时间戳从不设置或清除所有权；采样时刻的所有权仍只由 ledger 决定。
...
**逐一枚举输入设备。** 在滚轮 epoch 旁边加挂 `touchstart`/`pointerdown`/`keydown` 监听器是最显而易见的扩展。作为归因机制否决：拖动原生滚动条在其滚动事件到达之前不暴露任何可供锁存的输入事件；设备清单会随浏览器新增输入来源而逐渐腐化；而且每个监听器都需要自己的合成器交付宽限窗口——这正是 sticky-composer 笔记当初就拒绝构建的那个输入状态机。手势时间戳只为上文决策中「采样待处理」这一个问题回归，那时还没有可归因的已交付位置；它从不对已交付位置进行分类。
