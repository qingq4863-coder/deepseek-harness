...
A trusted-gesture timestamp answers one question the ledger cannot: whether a pending scroll sample is the reader's own move. `readerGestureAtRef` records `wheel`, `touchstart`, `touchmove`, `pointerdown`, and `keydown` on the scrollport, and `followRef` follows content growth during a pending sample unless a gesture arrived within `READER_GESTURE_WINDOW_MS` (1 s) — the case the [Chat fold and reopen note](2026-09-09-chat-fold-and-reopen-tail.md) fixes. The timestamp never sets or clears ownership; the ledger alone decides that at sample time.
...
**Enumerate input devices.** Adding `touchstart`/`pointerdown`/`keydown` listeners beside the wheel epoch was the obvious extension. Rejected as the attribution mechanism: native-scrollbar dragging exposes no input event to latch before its scrolls arrive, device lists rot as browsers add sources, and every listener would need its own compositor-delivery grace window — the input state machine the sticky-composer note already declined to build. A gesture timestamp returns only for the pending-sample question in the Decision above, where no delivered position exists to attribute yet; it never classifies a delivered position.
