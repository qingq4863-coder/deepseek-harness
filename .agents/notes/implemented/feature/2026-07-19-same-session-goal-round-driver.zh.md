...
该插件的配置就是它自己拥有的停止策略，因为决定是否继续目标的是它：`maxConsecutiveFailures`，连续失败停止上限；以及 `maxApprovalDenials`，驱动器自己的轮次处于已准入状态时用户拒绝的审批计数。两者都是带声明默认值的受校验插件字段，且插件会在安装任何东西之前拒绝不可用的取值。`maxGoalRounds` 由 `dsh-goal` 解析并持久化；「相同阻塞条件」的门槛由 `dsh-tool-goal` 解析并写入提示词。若驱动器重复声明这两个可调值中的任何一个，一个策略就会出现多个所有者。
...
`tests/loader-composition.spec.ts` 通过 Loader 启动一份测试专用 `cordis.yml`，并在真实会话存储、会话投影、提示词、工具、agent 与目标服务之上组合。它断言：组合后的驱动器会为当前目标预留一个 Goal Round，其内容恰好是该包自有的提示词；写在文档中的停止策略被接受；不可用的上限会在这行装载时失败。
